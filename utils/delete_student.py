...
def del_std():
    ...
    delStd = Tk()
    delStd.resizable(False,False)
    delStd.geometry("940x600+160+100")
    delStd.title("Delete Students")
    delStd.wm_iconbitmap("media/delete.ico")
    
    bg_img = Image.open("media/add_back.jpg")
    bg = ImageTk.PhotoImage(bg_img, master=delStd)
    
    back = Label(delStd, image=bg)
    back.pack()
    
    roll_no = tk.StringVar(delStd)
    name = tk.StringVar(delStd)
    division = tk.StringVar(delStd)
    section = tk.StringVar(delStd)
    gender = tk.StringVar(delStd)
    contact = tk.StringVar(delStd)
    email = tk.StringVar(delStd)
    dob = tk.StringVar(delStd)
    searchtext = tk.StringVar(delStd)
    searchby = tk.StringVar(delStd)
    ...
    display = Frame(delStd , bg="lavender blush" , width = 300 , height = 600 ,
                    highlightbackground="black" , highlightthickness=1)
    display.place(x=10,y=30)
    
    right = Frame(delStd,bg="snow",width=680,height=570)
    ...
    def delete_data():
        ...
        conn = mysql.connector.connect(host="127.0.0.1",user="root",password=passwd,database=dbname)
        cur = conn.cursor()
        opt = messagebox.askquestion("Confirm","Are you sure you want to delete ?")
        
        if opt=="yes":
            cur.execute("delete from "+dbname+" where roll_no = %s ",(roll_no.get(),))
            conn.commit()
            conn.close()
            fetch_data()
            clear()
    ...
    btn_del = Button(right,text="Delete",command=delete_data ,font=("times new roman","14","bold"),
                    relief=SOLID, bg="lavender blush" , fg="#be2102" , width=10)
    btn_del.place(x=250,y=530)
    delStd.mainloop()